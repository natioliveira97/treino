...
class LightningModule(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.metrics['name']        = ['accuracy', 'precision', 'recall', 'f1', 'aucroc']
        ...
        for split in self.metrics['split']:
            for i, category in enumerate(self.metrics['category']):
                ...
                self.metrics['torchmetric'][split+category+self.metrics['name'][4]] = torchmetrics.AUROC(task='binary',multiclass=False, mdmc_average=mdmc_average)
    ...
    def training_step(self, batch, batch_idx):
        ...
        return {'loss':total_loss, 'tile_labels':tile_labels, 'tile_preds':tile_preds}
        ...
        for image_names, image_losses, tile_probs, tile_preds, image_preds, image_probs, tile_labels in test_step_outputs:
            ...
            for image_name, image_loss, tile_prob, tile_pred, image_pred, image_prob, tile_label in zip(image_names, image_losses, tile_probs, tile_preds, image_preds, image_probs, tile_labels):
                ...
                if self.logger is not None:
                    ...
                    image_preds_csv_writer.writerow([image_name, image_pred, image_prob, image_loss, tile_probs, tile_pred, tile_labels])
