...
class MainModel(nn.Module):
    ...
    def forward_pass(self, x, tile_labels, bbox_labels, ground_truth_labels, omit_masks, split, num_epoch, device, outputs=None):
        ...
        Args:
            ...
            - device: current proce being used to create new torch objects without getting errors
        Outputs:
        ...
        for i, model in enumerate(self.model_list):
            ...
            # OBJECT DETECTION ONLY: If x is a dictionary of object detection losses...
                ...
                # Else for val/test loss...
                    ...
                    image_preds = torch.as_tensor([(output['scores'] > self.confidence_threshold).sum() > 0 for output in outputs[:len(ground_truth_labels)]]).to(device).float()
                    image_probs = torch.as_tensor([max(output['scores'])  for output in outputs[:len(ground_truth_labels)]]).to(device)
                    # Use number of errors as loss
                    total_loss = torch.abs(image_preds.float() - ground_truth_labels.float()).sum()
                    
                return losses, image_loss, total_loss, tile_probs, tile_preds, image_preds,image_preds
